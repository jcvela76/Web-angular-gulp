...
.controller('StoreController', function($stateParams, $state) {
		var self = this;
		console.log("working about")
		self.name = "Julio Vela";


	})

.controller('HomeComtroller', function($state, $document, $window, $interval, $timeout){
	var self = this;
	self.name = 'Julio';
...
// Part to scroll
  ...
  self.open_menu_1 = function(e) {
    if (!self.isActive){
      ...
      angular.element(close_ico).css( "display", "inline-block");
      $state.go('home.about');
  ...
  self.close_menu = function(e) {
    if (self.isActive){
      //go to the top is the user have the scroll in the bottom
      ...
      angular.element(close_ico).css( "display", "none");
  ...
  function center_background () {
    ...
    console.log("center_background", marRight);
